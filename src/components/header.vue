...
<template>
    <ul class="flex flex-row w-11/12 justify-end max-sm:w-full">
        <li v-for="item in navItems" :key="item.i18n"
            class="nav-item text-gray-200 px-3 p-5 border-b max-sm:border-transparent max-sm:hidden hover:text-gray-100"
            :class="{ ' border-sky-400': isCurrentPage(item.link) }">
            <a :href="item.link" class="flex flex-row h-6 text-lg items-center dark:text-gray-200">
                <span :class="{ ' text-gray-50 dark:text-blue-400': isCurrentPage(item.link) }" class="mr-1 h-6"
                    v-html="item.icon"></span>
                <span :class="{ 'font-bold  text-gray-50  dark:text-blue-400': isCurrentPage(item.link) }">{{ item.text
                }}</span>
            </a>
        </li>
        <li class="nav-item text-gray-200 px-3 p-5 list-none flex sm:hidden border-b border-transparent">
        ...
        <li class="nav-item text-gray-200 px-3 p-5 list-none flex border-b border-transparent">
            <button class="w-6 flex" type="button " @click="toggleDarkMode">

                <svg v-show="!isDarkMode" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                    viewBox="0 0 512 512">
                    <path

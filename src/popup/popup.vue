  ...
  <main class="dark:bg-gray-200 min-w-[640px]  min-h-[500px] h-screen">
    ...
    <div class="w-full p-8">
      <div class="flex justify-center">
        <Dropzone class="w-full max-w-5xl"></Dropzone>
      </div>

    </div>
    <div class="w-full p-8">
      <div class="flex justify-center">
        <div class="flex flex-row shadow rounded-lg max-sm:flex-col w-full max-w-6xl border dark:shadow-lg dark:border-gray-300">
          <div class="p-1 text-center flex flex-col border-r-2 w-36 max-sm:flex-row max-sm:w-full max-sm:justify-center dark:border-gray-300">
            <button class="p-4 hover:bg-slate-100 hover:text-blue-900 border-b-2 truncate dark:border-gray-300" type="button"
              v-for="option in urlType" :key="option.value"
              :class="{ 'border-b-2 border-blue-400 text-blue-900 font-bold': selectedType === option.value }"
              v-on:click="selectType(option.value)">
              {{ option.label }}
            </button>
          </div>
              ...
              <div class="relative flex flex-row my-1 shadow border p-1 hover:bg-blue-100 dark:border-gray-300">
                <div class="truncate">
                  <p class="p-2 shadow truncate text-ellipsis url-text" @click="getUrlText($event)">
                    上传成功后URL将会显示在这里上传成功后URL将会显示在这里上传成功后URL将会显示在这里上传成功后URL将会显示在这里上传成功后URL将会显示在这里上传成功后URL将会显示在这里上传成功后URL将会显示在这里上传成功后URL将会显示在这里上传成功后URL将会显示在这里上传成功后URL将会显示在这里上传成功后URL将会显示在这里上传成功后URL将会显示在这里上传成功后URL将会显示在这里上传成功后URL将会显示在这里上传成功后URL将会显示在这里上传成功后URL将会显示在这里上传成功后URL将会显示在这里上传成功后URL将会显示在这里上传成功后URL将会显示在这里上传成功后URL将会显示在这里上传成功后URL将会显示在这里上传成功后URL将会显示在这里上传成功后URL将会显示在这里
                  </p>
                </div>
